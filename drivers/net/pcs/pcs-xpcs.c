...
static const struct dw_xpcs_compat baikal_xpcs_compat[] = {
	{
		.interface = PHY_INTERFACE_MODE_XGMII,
		.supported = xpcs_xgmii_features,
		.an_mode = DW_AN_C73,
...
static const struct dw_xpcs_desc xpcs_desc_list[] = {
	{
		...
		.id = BAIKAL_XGMAC_XPCS_ID,
		.mask = DW_XPCS_ID_MASK,
		.compat = baikal_xpcs_compat,
	},
};
...
static int xpcs_init_ctrl(struct dw_xpcs *xpcs)
{
	int ret;
