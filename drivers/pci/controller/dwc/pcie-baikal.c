...
static int baikal_pcie_host_init(struct dw_pcie_rp *pp)
{
	...
	if (!linkup) {
		...
		/* Assert PERST pin */
		rc->reset_gpio = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_HIGH);
		ret = PTR_ERR_OR_ZERO(rc->reset_gpio);
		if (ret) {
			dev_err(dev, "unable to request reset GPIO: %d\n", ret);
			rc->reset_gpio = NULL;
		} else {
			snprintf(rc->reset_name, sizeof(rc->reset_name), "pcie%u-reset", rc->num);
			ret = gpiod_set_consumer_name(rc->reset_gpio, rc->reset_name);
			if (ret) {
