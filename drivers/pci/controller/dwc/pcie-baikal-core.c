...
static int bm1000_pcie_init(struct pci_config_window *cfg)
{
	...
	pci->max_link_speed = 3;
	dev->fwnode = &to_acpi_device(dev)->fwnode;
...
static irqreturn_t bs1000_pcie_intr_irq_handler(int irq, void *arg)
{
	...
	root_err     = dw_pcie_readl_dbi(pci,
					 ext_cap_err_off + PCI_ERR_ROOT_STATUS);
	root_err_src = dw_pcie_readl_dbi(pci,
					 ext_cap_err_off + PCI_ERR_ROOT_ERR_SRC);
	dev_sta	     = dw_pcie_readw_dbi(pci,
					 exp_cap_off + PCI_EXP_DEVSTA);
	...
	if (uncor_err == PCI_ERR_UNC_COMP_TIME) {
		if (root_err & PCI_ERR_ROOT_MULTI_UNCOR_RCV) {
			dev_err(dev,
				"uncorrectable error: completion timeout\n");
		} else {
			dev_err(dev,
				"uncorrectable error: completion timeout (RID:0x%x)\n",
				(root_err_src >> 16) & 0xffff);
		}
	} else if (root_err == PCI_ERR_ROOT_COR_RCV) {
		/* Skip 'dev_err' message for a single correctable error */
	} else {
		dev_err(dev,
