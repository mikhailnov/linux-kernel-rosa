...
#define ECCSTAT_CORR_ERR	(0xf << 8)
#define ECCSTAT_UNCORR_ERR	(0xf << 16)
...
static struct mem_ctl_info *mcis_list[12];
static struct timer_list *tmrs_list[12];
...
static struct mem_ctl_info *mci_from_timer(struct timer_list *timer)
{
	for (int i = 0; i < 12; i++) {
		if (tmrs_list[i] == timer)
			return mcis_list[i];
	}
	return NULL;
}

static void baikal_edac_timer_callback(struct timer_list *timer)
{
	static u16 ue_cntr, ce_cntr;
	u32 eccstat, ecccntr, regaddr0, regaddr1;
	struct mem_ctl_info *mci = mci_from_timer(timer);
	struct baikal_edac_priv *priv = mci->pvt_info;
	eccstat = readl(priv->baseaddr + DDRC_ECCSTAT);
	ecccntr = readl(priv->baseaddr + DDRC_ECCERRCNT);

	if (eccstat & ECCSTAT_CORR_ERR) {
		...
		snprintf(priv->msg, BAIKAL_EDAC_MSG_SIZE,
			 "catched at Rank: %d, BankGroup: %d, Bank: %d, Row: %d, Col: %d; Bitmask: 0x%llx\n",
			 ...
			 (u64)readl(priv->baseaddr + DDRC_ECCBITMASK0) << 32 |
			 readl(priv->baseaddr + DDRC_ECCBITMASK1));

		edac_mc_handle_error(HW_EVENT_ERR_CORRECTED, mci,
				     (ecccntr & 0xffff) - ce_cntr, 0, 0, (u64)readl(priv->baseaddr + DDRC_ECCCSYN0) << 32 |
	...
	if (eccstat & ECCSTAT_UNCORR_ERR) {
		...
		edac_mc_handle_error(HW_EVENT_ERR_UNCORRECTED, mci,
				     (ecccntr >> 16) - ue_cntr, 0, 0, (u64)readl(priv->baseaddr + DDRC_ECCUSYN0) << 32 |
...
static int baikal_mc_probe(struct platform_device *pdev)
{
	...
	if (readl(baseaddr + DDRC_CRCPARCTL0)) {
		...
		if (priv->irq_dfi < 0) {
			if (priv->irq_dfi == -ENXIO) { /* 2 SoC */
				edac_printk(KERN_INFO, EDAC_MC,
				    "Interrupt property was deleted (%d)\n",
					    priv->irq_dfi);
				goto polling_routine;
			}
			edac_printk(KERN_ERR, EDAC_MC,
				    "No IRQ in DT (%d)\n", priv->irq_dfi);
			ret = priv->irq_dfi;
...
polling_routine:
	tmrs_list[mc_idx] = kzalloc(sizeof(struct timer_list), GFP_KERNEL);
	if (tmrs_list[mc_idx] == NULL) {
		edac_printk(KERN_ERR, EDAC_MC, "Unable to allocate timer\n");
		ret = -ENOMEM;
		goto free_mc;
	}
	mcis_list[mc_idx] = mci;

	timer_setup(tmrs_list[mc_idx], baikal_edac_timer_callback, 0);
	mod_timer(tmrs_list[mc_idx], jiffies + msecs_to_jiffies(5000));
...
static void baikal_mc_remove(struct platform_device *pdev)
{
	struct mem_ctl_info *mci = platform_get_drvdata(pdev);
	struct baikal_edac_priv *priv = mci->pvt_info;

	if (tmrs_list[mci->mc_idx]) {
		del_timer_sync(tmrs_list[mci->mc_idx]);
		kfree(tmrs_list[mci->mc_idx]);
