...
static int altha_list_handler(const struct ctl_table *table, int write,
			      ...
			      loff_t * ppos)
{
	...
	if (write && !error) {
		...
		strscpy(copy_buffer, table->data, ALTHA_PARAMS_SIZE);
...
const struct lsm_id altha_lsmid = {
	.name = "altha",
	.id = LSM_ID_ALTHA
};
...
static int __init altha_init(void)
{
	if (altha_enabled) {
		...
		security_add_hooks(altha_hooks, ARRAY_SIZE(altha_hooks), &altha_lsmid);
